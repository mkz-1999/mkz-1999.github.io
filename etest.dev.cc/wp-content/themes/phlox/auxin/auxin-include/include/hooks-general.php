...
function auxin_wp_ulike_number_format( $value, $num, $plus ){
    ...
    if( ( 'portfolio' == $post->post_type ) && ( is_single( $post ) ) ){
        ...
        return ( auxin_get_option( 'portfolio_single_like_button_type', false ) == 'icon' ) ? __( 'Likes', 'phlox' ) . ' (' . $value . ')' : $value ;
    }
...
function auxin_modern_comment_form( $comment_form, $params ){
    ...
    $reply_title   = auxin_get_option( 'comment_forms_replay_title', __( 'Add a Comment', 'phlox' ) );
